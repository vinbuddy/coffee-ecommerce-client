...
export default async function AdminProductDetailPage({ params }: { params: { id: string } }) {
    let [productData, productToppingData, productSizeData] = await Promise.all([
        ...
        fetchData(`${process.env.NEXT_PUBLIC_API_BASE_URL}/product/product-toppings/${params.id}`),
        fetchData(`${process.env.NEXT_PUBLIC_API_BASE_URL}/product/product-sizes/${params.id}`),
    ]);
        ...
        <div>
            ...
            <div className="grid grid-cols-12 gap-5">
                <section className="col-span-12 sm:col-span-12 md:col-span-7 lg:col-span-7 xl:col-span-7 2xl:col-span-7">
                    <div>
                        <h1 className="font-bold text-3xl">{product?.name || "Unknown name"}</h1>

                        <p className="text-primary text-xl mt-3">
                            {formatVNCurrency(product?.price) || "Unknown price"}
                        </p>
                        <Chip className="mt-3" color="default" variant="dot">
                            {product?.category_name || "Unknown category"}
                        </Chip>
                        <p className="mt-3 text-gray-500">{product?.description || "Unknown description"}</p>

                        <div className="mt-5 border-t py-5">
                            {productSizes.length > 0 && (
                                <div className="mb-5">
                                    <h3 className="text-default-500 mb-3">Size của sản phẩm</h3>
                            ...
                            {productToppings.length > 0 && (
                                <div>
                                    <h3 className="text-default-500 mb-3">Topping của sản phẩm</h3>
                                    <div className="flex gap-3">
                                        {productToppings.map((topping) => (
                                            <Chip variant="flat" key={topping.id}>
                ...
                <section className="col-span-12 sm:col-span-12 md:col-span-5 lg:col-span-5 xl:col-span-5 2xl:col-span-5">
                    <div>
                        <Image className="w-full h-auto" src={product?.image || ""} alt="" />
                    </div>
                </section>
            </div>
        </div>
